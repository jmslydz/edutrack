...
$direction = isset($direction) ? $direction : 'sent'; // 'sent' | 'received'

$flash_success = $this->session->flashdata('grade_success');
$flash_error   = $this->session->flashdata('grade_error');

// Build the "from → to" summary line
if ($direction === 'sent')
{
    if ($ticket->recipient_type === 'admin')
    {
        $party_line = 'You &rarr; Admin';
    }
    else
    {
        $rname = trim($ticket->recipient_first_name . ' ' . $ticket->recipient_last_name);
        $party_line = 'You &rarr; ' . html_escape($rname !== '' ? $rname : 'Instructor');
    }
}
else
{
    $sname = trim($ticket->submitter_first_name . ' ' . $ticket->submitter_last_name);
    $party_line = html_escape($sname !== '' ? $sname : 'Instructor') . ' &rarr; You';
}
?>
<!-- BEGIN PAGE CONTENT -->
  <div class="page-header">
    <div>
      ...
      <p class="page-subtitle">
        <?php echo html_escape($ticket->category); ?> &nbsp;&middot;&nbsp;
        <span class="badge <?php echo $ticket->status === 'Open' ? 'badge-success' : ($ticket->status === 'In Progress' ? '' : 'badge-neutral'); ?>"><?php echo html_escape($ticket->status); ?></span>
        &nbsp;&middot;&nbsp;
        <span style="font-size:0.78rem; color:#64748B;"><?php echo $party_line; ?></span>
      </p>
    </div>
    <a href="<?php echo site_url('student/tickets'); ?>" class="btn-secondary" style="display:flex; align-items:center; gap:6px; border-radius:12px; border:none; cursor:pointer; text-decoration:none; font-size:0.8rem;">
  ...
  <div class="card" style="margin-bottom:24px;">
    <div style="padding:24px; border-bottom:1px solid #F1F5F9;">
      <div style="display:flex; gap:12px; flex-wrap:wrap; margin-bottom:12px;">
        ...
        <?php if ($direction === 'received'): ?>
          <span class="badge" style="background:#FDF4FF;color:#9333EA;font-size:0.7rem;">Message from Instructor</span>
        <?php endif; ?>
      ...
      <p style="margin:12px 0 0; color:#334155; font-size:0.9rem; line-height:1.6; white-space:pre-wrap; word-wrap:break-word; overflow-wrap:break-word;"><?php echo html_escape($ticket->message); ?></p>
    </div>
  </div>
            ...
            <div style="flex:1;">
              <div style="display:flex; align-items:center; gap:8px; margin-bottom:4px; flex-wrap:wrap;">
                ...
                <?php elseif ($r->role === 'teacher'): ?>
                  <span class="badge" style="background:#F0FDFA;color:#0D9488;font-size:0.65rem;">Instructor</span>
                <?php else: ?>
                  <span class="badge" style="background:#EFF6FF;color:#2563EB;font-size:0.65rem;">Student</span>
                <?php endif; ?>
              </div>
              <p style="margin:0; color:#334155; font-size:0.9rem; line-height:1.6; white-space:pre-wrap; word-wrap:break-word; overflow-wrap:break-word;"><?php echo html_escape($r->message); ?></p>
            </div>
          </div>
        <?php endforeach; ?>
      </div>
  ...
  <div class="card">
    ...
    <?php echo form_open('student/ticket_reply/' . (int) $ticket->id, array('novalidate' => 'novalidate')); ?>
