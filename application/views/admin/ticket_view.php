...
<!-- BEGIN PAGE CONTENT -->
  <div class="page-header">
    <div>
      ...
      <p class="page-subtitle"><?php echo html_escape($ticket->category); ?> · Submitted by <?php echo html_escape($ticket->first_name . ' ' . $ticket->last_name); ?> (<?php echo html_escape($ticket->email); ?>, <?php echo html_escape($ticket->role); ?>)</p>
    </div>
    <a href="<?php echo site_url('admin/tickets'); ?>" class="btn-secondary" style="display:flex; align-items:center; gap:6px; border-radius:12px; border:none; cursor:pointer; text-decoration:none; font-size:0.8rem;">
  ...
  <div class="card" style="margin-bottom:24px;">
    <div style="padding:24px; border-bottom:1px solid #F1F5F9;">
      <div style="display:flex; gap:12px; flex-wrap:wrap; margin-bottom:12px;">
        ...
        <span class="text-faint" style="font-size:0.75rem;">Submitted <?php echo date('M j, Y g:i A', strtotime($ticket->created_at)); ?></span>
      ...
      <div style="margin-bottom:12px;">
        <strong style="color:#1E293B;">Submitted by:</strong> <?php echo html_escape($ticket->first_name . ' ' . $ticket->last_name); ?> 
        <span style="color:#64748B; font-weight:normal;">(<?php echo html_escape($ticket->email); ?>, <?php echo html_escape($ticket->role); ?>)</span>
      </div>
          ...
          <div style="display:flex; gap:12px;">
            ...
            <div style="flex:1;">
              <div style="display:flex; align-items:center; gap:8px; margin-bottom:4px;">
                ...
                <?php else: ?>
                  <span class="badge" style="background:#F0FDFA;color:#0D9488;font-size:0.65rem;"><?php echo html_escape($r->role); ?></span>
                <?php endif; ?>
              </div>
  ...
  <div class="card" style="margin-bottom:24px;">
    <div style="padding:16px 24px; border-bottom:1px solid #F1F5F9;">
      <h3 style="font-family:var(--font-display); font-size:1rem; font-weight:700; color:#1E293B; margin:0;">Reply & Update Status</h3>
    </div>
    <?php echo form_open('admin/ticket_view/' . (int) $ticket->id, array('novalidate' => 'novalidate')); ?>
    ...
    <div style="padding:24px;">
      ...
      <div class="form-group" style="margin-top:16px;">
        <label class="form-label" for="status">Status (defaults to current)</label>
        <select class="form-input" id="status" name="status">
          <option value="Open" <?php echo $ticket->status === 'Open' ? 'selected' : ''; ?>>Open</option>
          <option value="In Progress" <?php echo $ticket->status === 'In Progress' ? 'selected' : ''; ?>>In Progress</option>
          <option value="Resolved" <?php echo $ticket->status === 'Resolved' ? 'selected' : ''; ?>>Resolved</option>
        </select>
        <div class="text-faint" style="font-size:0.75rem;margin-top:4px;">Status will be updated together with your reply</div>
      </div>
      <div style="display:flex; gap:12px; margin-top:24px; justify-content:flex-end;">
        <button type="submit" class="btn-primary" style="flex:1; max-width:200px;">Send Reply & Update</button>
      </div>
    </div>
    <?php echo form_close(); ?>
  </div>